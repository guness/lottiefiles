...
/**
 * Created by guness on 17.11.2021 10:18
 */
@Composable
fun MoodTrackerCard(modifier: Modifier = Modifier) {
    Card(modifier = modifier.defaultMinSize(minHeight = 180.dp)) {
        Column {
            Body1(
                text = stringResource(id = R.string.profile_mood_tracker),
                modifier = Modifier.padding(Padding.m),
                color = MaterialTheme.colors.primary
            )

            Title2(
                text = stringResource(id = R.string.profile_no_mood),
                modifier = Modifier
                    .padding(Padding.m)
